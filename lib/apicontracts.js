...
class ListOfAUDetailsType {
	constructor(obj) {
		...
		if(arguments.length == 1) {
			if((obj != undefined) && (obj != null)) {
				var auUpdateList = [];
				obj.forEach(function(item){if(item.newCreditCard != null) {auUpdateList.push(new AuUpdateType(item));}}); 
				this.setAuUpdate(auUpdateList);
				var auDeleteList = [];
				obj.forEach(function(item){if(item.creditCard != null) {auDeleteList.push(new AuDeleteType(item));}}); 
				this.setAuDelete(auDeleteList);
			}
		}
		else {
			this.setAuUpdate(null);
			this.setAuDelete(null);
		}
		logger.debug('Exit ListOfAUDetailsType constructor');
	}

	setAuUpdate(p_auUpdate) { this.auUpdate = p_auUpdate; }
	getAuUpdate() { if('auUpdate' in this) {return this.auUpdate;} }
	setAuDelete(p_auDelete) { this.auDelete = p_auDelete; }
	getAuDelete() { if('auDelete' in this) {return this.auDelete;} }
}
...
class AuDeleteType extends AuDetailsType {
	constructor(obj) {
		...
		if(arguments.length == 1) {
			super(obj);
			if(('creditCard' in obj) && (obj.creditCard != null)) { this.setCreditCard(new CreditCardMaskedType(obj.creditCard)); }
			if(('subscriptionIdList' in obj) && (obj.subscriptionIdList != null)) { this.setSubscriptionIdList(new SubscriptionIdList(obj.subscriptionIdList)); }
		}
		else {
			super();
			this.setCreditCard(null);
			this.setSubscriptionIdList(null);
		}
	...
	setCreditCard(p_creditCard) { this.creditCard = p_creditCard; }
	getCreditCard() { if('creditCard' in this) {return this.creditCard;} }
	setSubscriptionIdList(p_subscriptionIdList) { this.subscriptionIdList = p_subscriptionIdList; }
	getSubscriptionIdList() { if('subscriptionIdList' in this) {return this.subscriptionIdList;} }
}
